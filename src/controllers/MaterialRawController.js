...
class MaterialRawController {
  ...
  async consumeOutput(req, res) {
    try {
      const result = await Material.sequelize.query(`select DATE(o.created_at) as createdAtOut, DATE_FORMAT(DATE(o.created_at),'%d/%m/%Y') as createdAtOutBr, oi.material_id as materialIdOut, o.req_maintenance as reqMaintenance, o.place as place, w.name as workerName, u.username as authorizedBy, materials.name, materials.unit, SUM(oi.quantity) as sumQuantityOut, CONCAT('R$ ',FORMAT(MAX(oi.value), 2, 'pt_BR')) as maxPriceValueOut, SUM(oi.quantity)*MAX(oi.value) as totalPrice, iiReturn.return_id, ifnull(SUM(iiReturn.quantity),0) as SumQuantityReturned, ifnull((SUM(oi.quantity) - SUM(iiReturn.quantity)),SUM(oi.quantity)) as sumEffectiveQuantityOut, COUNT(oi.material_id) as outputFrequency from materials_out_items as oi left join materials_out as o on oi.material_out_id = o.id left join workers as w on o.worker_id=w.id left join users as u on o.authorized_by=u.id left join materials on oi.material_id=materials.id left join (select ii.material_id as materialIdIn, ii.quantity, i.req_maintenance, DATE(i.created_at) as createdAtIn, i.return_id from materials_in_items as ii left join materials_in as i on ii.material_in_id = i.id where i.material_intype_id=3) as iiReturn on o.id = iiReturn.return_id where o.material_outtype_id=1 group by materialIdOut, reqMaintenance, createdAtOut, workerName having (createdAtOut BETWEEN CAST('${req.body.startDate}' AS DATE) AND CAST('${req.body.endDate}' AS DATE) AND sumEffectiveQuantityOut>0) order by DATE(createdAtOut)`, { type: QueryTypes.SELECT });
      return res.json(result);
    } catch (e) {
      return res.status(400).json({
        errors: [e.message],
